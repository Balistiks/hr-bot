...
async def create_excel_applicant(tgid):
    ...
    for applicant in applicants:
        data_formatted.append({
            ...
            'tgid': applicant['tgId'],
            'Дата первого сообщения пользователя': datetime.datetime.strptime(
                applicant['startDate'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime(
                '%d.%m.%Y'
            ) if applicant['startDate'] is not None else '',
            'Дата регистрации пользователя': datetime.datetime.strptime(
                applicant['registeredAt'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime(
                '%d.%m.%Y'
